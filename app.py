...
# --- 2. AUTHENTICATION (Using your existing Secrets) ---
def get_sheet():
    scope = ["https://www.googleapis.com/auth/spreadsheets", "https://www.googleapis.com/auth/drive"]
    # Maps your TOML secrets to the required dictionary format
    creds_dict = {
...
with st.sidebar:
    ...
    with st.form("entry_form", clear_on_submit=True):
        ...
        p3 = st.selectbox("Wildcard", WILDCARD)
        if st.form_submit_button("SUBMIT") and u_name:
            try:
                sheet = get_sheet()
...
try:
    ...
    if entries:
        results = []
        for row in entries:
            score = sum(player_map.get(str(row[p]).lower(), 0) for p in ["P1", "P2", "P3"])
            results.append({"User": row["User"], "Total": score})
        st.table(pd.DataFrame(results).sort_values("Total"))
    else:
